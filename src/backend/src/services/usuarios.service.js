...
export default class UsuariosService {
  ...
  async Cadastrar(dados) {
    try {
      ...
      if (resultUsuario.length) {
        throw new Error(
          JSON.stringify({ erro: "Usuário já cadastrado!", status: 400 }),
        );
      }
      ...
      if (resultadoTesteSenha) {
        throw new Error(
          JSON.stringify({ erro: resultadoTesteSenha, status: 400 }),
        );
      }

      const senhaCriptografada = await criptografar(senha);
      const response = await usuariosRepository.Cadastrar(
        nome,
        email,
        senhaCriptografada,
        perfil,
  ...
  async Deletar(id) {
    try {
      if (!id) {
        throw new Error(
          JSON.stringify({ erro: "ID está faltando!", status: 400 }),
        );
      }
  ...
  async Atualizar(id, dados) {
    try {
      ...
      if (!dados) {
        throw new Error(
          JSON.stringify({
            erro: "Preencha os dados para a atualização!",
            status: 400,
          }),
        );
      }
      ...
      if (dados.email) {
        const emailCadastrado = await usuariosRepository.BuscarUsuario(
          dados.email,
        );

        if (emailCadastrado.length)
          throw new Error(
            JSON.stringify({ erro: "Email já cadastrado!", status: 400 }),
          );
      }
